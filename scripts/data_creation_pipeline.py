...
def main():
    ...
    argparser.add_argument('idx_LH_start', type=int)
    argparser.add_argument('idx_LH_end', type=int, nargs='?', default=None)  
    args = argparser.parse_args()
    idx_LH_end = args.idx_LH_end
    if idx_LH_end is None:
        idx_LH_end = args.idx_LH_start + 1
    for idx_LH in range(args.idx_LH_start, idx_LH_end):
        run(idx_LH)
...
def run(idx_LH):
    ...
    if run_zspace:
        if os.path.exists(fn_bfields_kcut_deconvolved) and \
           ...
           not overwrite_bfields:
            print(f"Real-space and z-space bias fields (deconvolved, kcut) for LH {idx_LH} already exists and overwrite_bfields = {overwrite_bfields}, our work here is done!", flush=True)
        ...
        if os.path.exists(fn_bfields_kcut_deconvolved) and not overwrite_bfields:
            print(f"Bias field (deconvolved, kcut) for LH {idx_LH} already exists and overwrite_bfields = {overwrite_bfields}, our work here is done!", flush=True)
            return
    ...
    if run_zspace:
        ...
        predicted_positions_to_bias_fields(n_grid, n_grid_target, box_size, sim, 
                                        ...
                                        save_hr_field, deconvolve_lr_field, save_intermeds,
                                        dir_LH, idx_LH, tag_bfields=tag_zspace)
    ...
    timenow = time.time()
    print(f"TIME TOTAL for LH {idx_LH}: {timenow-start} s", flush=True)
    return
...
def predicted_positions_to_bias_fields(n_grid, n_grid_target, box_size, sim, 
    ...
    if deconvolve_lr_field:
        ...
        fn_bfields_kcut_deconvolved = f'{dir_LH}/bias_fields_eul{tag_bfields}_deconvolved_{idx_LH}.npy'

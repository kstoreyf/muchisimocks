...
class MomentNetworkDL(): 

    def __init__(self, ds_train=None, ds_val=None, ds_test=None,
        ...
        if self.ds_train is not None:
            y0, theta0 = next(iter(self.ds_train.take(1)))
            self.n_dim = y0.shape
            self.n_params = len(theta0)
            print("n_params =", self.n_params)
        elif self.ds_test is not None:
            y0, theta0 = next(iter(self.ds_test.take(1)))
            self.n_dim = y0.shape
            self.n_params = len(theta0)
    ...
    def fit_network_mean(self, max_epochs=1500):
        ...
        print("Fitting model")
        hist = self.model_mean.fit(self.ds_train,
                                epochs=max_epochs, batch_size=32, shuffle=True,
                                callbacks=[callback],

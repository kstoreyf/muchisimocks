...
#tag_stats_arr=("_pk_bispec_pgm")
#tag_stats_arr=("_pk" "_pgm" "_pk_pgm" "_pk_bispec_pgm")
#tag_stats_arr=("_pk_bispec")
    ...
    for tag_stats in "${tag_stats_arr[@]}"; do
        ...
        if [[ "$config_train_file" == "none" && "$config_test_file" == "none" ]]; then
            ...
            if [[ "$data_mode" == "shame" ]]; then
                job_name="run_inf${tag_stats}${tag_params}${tag_Anoise}${tag_rp}_TEST${tag_data_test}"
            else
                job_name="run_inf${tag_stats}${tag_params}${tag_Anoise}${tag_rp}_TEST${tag_params_test}${tag_Anoise_test}"
            fi
...
#SBATCH --time=3:00:00 #0.5-2h for training -> 3 in case
